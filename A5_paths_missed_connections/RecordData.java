...
public class RecordData extends WritableSerialization{
	...
	private void settingDates(String[] flightRecord, int offset) throws BadDataException {
		...
		// converting to 00:00 standard time and adding one day/if arr time less than dep then must be next day
		if (cArrTime == 2400) {
			cArrTime = 0000;
		...
		cal.set(this.getYear(), this.getMonth(),
				this.getDay());
		// converting to 00:00 standard time and adding one day
		if (cDepTime == 2400) {
			cDepTime = 0000;
