...
public final class Util {
	...
	public static List<String> processDir(String dirPath) {
		...
		try (DirectoryStream<Path> directoryStream = Files
			...
			for (Path path : directoryStream) {
				if (path.toString().contains("csv.gz"))
	...
	/**
	 * method to see if the given date is close to a holiday by number of days
	 * specified in HOLIDAYGAP.
	 * 
	 * @param month
	 *            month
	 * @param day
	 ...
	 */
	public static int nearHoliday(int month, int day, int year) {
		Calendar c = new GregorianCalendar(year, month, day);
		c.set(Calendar.YEAR, year);
		for (Calendar h : Constants.HOLIDAYS) {
			if (Math.abs((timeDiff(h.getTime(), c.getTime()) / 60) / 24) <= Constants.HOLIDAYGAP)
				return 1;
		}
		return 0;
	}
	...
	/**
	 * get time difference in minutes (d1-d2)
	 * 
	 * @param d1
	 *            date 1
	 * @param d2
	 *            date 2
	 * @return Difference in minutes(d1-d2)
	 */
