...
public class RecordData extends WritableSerialization {
	...
	public int destAirportSeqId;
	public int originAirportSeqId;
	public int destCityMArketId;
	public int originCityMarketId;
	public int destStateFips;
	public int OriginStateFips;
	public int DestWac;
	public int OriginWac;
	public String origin;
	public String dest;
	public String originCityName;
	public String destCityName;
	public String originStateNm;
	public String originStateAbr;
	public String destStateNm;
	public String destStateAbr;
	...
	public boolean isSane() {
		...
		if (!checkTimes() || !checkOrigDestData() || !checkIDData()) {
			return false;
		}
	...
	private boolean checkNonCancelledData() {
		...
		long duration = this.getArrTime().getTime() - this.getDepTime().getTime();
	...
	private boolean checkTimes() {
		...
		long duration = this.getCrsArrTime().getTime() - this.getCrsDepTime().getTime();
		long timeDiff = TimeUnit.MILLISECONDS.toMinutes(duration);
	...
	private boolean checkIDData() {
		...
		// larger than 0
		if (this.getDestAirportId() <= Constants.ZERO || this.getOriginAirportId() <= Constants.ZERO
				|| this.getDestAirportSeqId() <= Constants.ZERO || this.getOriginAirportSeqId() <= Constants.ZERO
				|| this.getDestCityMArketId() <= Constants.ZERO || this.getOriginCityMarketId() <= Constants.ZERO
				|| this.getDestStateFips() <= Constants.ZERO || this.getOriginStateFips() <= Constants.ZERO
				|| this.getDestWac() <= Constants.ZERO || this.getOriginWac() <= Constants.ZERO) {
			return false;
		}
		return dataGood;
	}
	...
	private boolean checkOrigDestData() {
		...
		// empty
		if (this.getOrigin().isEmpty() || this.getDest().isEmpty() || this.getOriginCityName().isEmpty()
				|| this.getDestCityName().isEmpty() || this.getOriginStateNm().isEmpty()
				|| this.getOriginStateAbr().isEmpty() || this.getDestStateNm().isEmpty()
	...
	public void fill(String[] flightRecord, int offset) throws BadDataException {
		try {
			...
			this.setMonth(Integer.parseInt(flightRecord[Constants.MONTH + offset]));
			...
			this.arrDelay = Float.parseFloat(flightRecord[Constants.ARRDEL + offset]);
			if (flightRecord[Constants.CANCELLED + offset].equals("0")) {
				this.setCancelled(false);
				this.setPrice(Float.parseFloat(flightRecord[Constants.AVGPRICE + offset]));
				this.setActualElapsedTime(Integer.parseInt(flightRecord[Constants.ACTELAPTIME + offset]));
				this.setArrDel15(Float.parseFloat(flightRecord[Constants.ARRDEL15 + offset]));
				this.setArrDelay(Float.parseFloat(flightRecord[Constants.ARRDEL + offset]));
				this.setArrivalDelayNew(Float.parseFloat(flightRecord[Constants.ARRDELNEW + offset]));
			} else {
				this.setCancelled(true);
			}
			settingDates(flightRecord, offset);
			this.setCrsElapsedTime(Integer.parseInt(flightRecord[Constants.CRSELAPSED + offset]));
			this.setDest(flightRecord[Constants.DEST + offset]);
			this.setDestAirportId(Integer.parseInt(flightRecord[Constants.DESTAIRPORTID + offset]));
			this.setDestAirportSeqId(Integer.parseInt(flightRecord[Constants.DESTAIRPORTSEQID + offset]));
			this.setDestCityMArketId(Integer.parseInt(flightRecord[Constants.DESTMARKETID + offset]));
			...
			this.setDestStateFips(Integer.parseInt(flightRecord[Constants.DESTSTATEFIPS + offset]));
			this.setDestStateNm(flightRecord[Constants.DESTSTATENM + offset]);
			this.setDestWac(Integer.parseInt(flightRecord[Constants.DESTWAC + offset]));
			this.setOrigin(flightRecord[Constants.ORIGIN + offset]);
			this.setOriginAirportId(Integer.parseInt(flightRecord[Constants.ORIGINAIRPORTID + offset]));
			this.setOriginAirportSeqId(Integer.parseInt(flightRecord[Constants.ORIGINAIRPORTSEQID + offset]));
			this.setOriginCityMarketId(Integer.parseInt(flightRecord[Constants.ORIGINMARKETID + offset]));
			this.setOriginCityName(flightRecord[Constants.ORIGINCITYNAME + offset]);
			this.setOriginStateAbr(flightRecord[Constants.ORIGINSTATEABR + offset]);
			this.setOriginStateFips(Integer.parseInt(flightRecord[Constants.ORIGINSTATEFIPS + offset]));
			this.setOriginStateNm(flightRecord[Constants.ORIGINSTATENM + offset]);
			this.setOriginWac(Integer.parseInt(flightRecord[Constants.ORIGINWAC + offset]));
	...
	private void settingDates(String[] flightRecord, int offset) throws BadDataException {
		...
		if (!this.isCancelled()) {
			...
			cal.set(this.getYear(), this.getMonth(), this.getDay());
			int depTime = Integer.parseInt(flightRecord[Constants.DEPTIME + offset]);
			// converting to 00:00 standard time
			if (depTime == 2400) {
				depTime = 0000;
			}
			...
			int arrTime = Integer.parseInt(flightRecord[Constants.ARRTIME + offset]);
			// converting to 00:00 standard time
			if (arrTime == 2400) {
				arrTime = 0000;
			}
